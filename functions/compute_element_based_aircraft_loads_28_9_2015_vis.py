    ...
    for i in range(0,len(aircraft.main_wing)):
        ...
        for j in range(0,len(elemlist)):
            ...
            #compute the point direction relative to the wing root
                ...
                max_load   = 0.5*2.0*aircraft.main_wing[i].sizing_lift/aircraft.main_wing[i].span
                load_scale = max_load
...
#Visualize the results
    pressure = np.zeros(len(elemlist))
    y_position = np.zeros(len(elemlist))
    for i in range(0,len(elemlist)):
        
        pressure[i] = elemlist[i].pressure
        y_position[i] = elemlist[i].x[1]
